...
void* soma(void* arg){
    struct estrutura data = *(struct estrutura*)arg;
    for(int i = data.comeco; i<data.fim;i++){
...
int main(int argc, char* argv[]) {
    ...
    for(int i = 0; i<n_threads;i++){
        struct estrutura *dado = malloc(sizeof(struct estrutura));
        dado->a=a;
        dado->b=b;
        dado->c=c;
        int div = a_size/n_threads;
        dado->comeco = i*div;
        dado->fim = dado->comeco+div;

        pthread_create(&vetor[i],NULL,soma,(void*)dado);
    }
